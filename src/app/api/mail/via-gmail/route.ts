  ...
  try {
    ...
    if (!name || !emails || !Array.isArray(emails) || emails.length === 0 || !subject || !message) {
      return NextResponse.json(
        { error: 'All fields (name, emails, subject, message) are required, and emails should be an array with at least one email.' },
        { status: 400 }
      );
    }
 
    if (!EMAIL_ID || !EMAIL_PASS) {
      return NextResponse.json({ error: 'Email credentials are missing in environment variables' }, { status: 500 });
    }
